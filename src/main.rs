...
use std::{array, convert::TryInto, path::Path, sync::{Arc, Mutex}};
...
use gtk::{*, DialogExt, WidgetExt, prelude::*};
...
async fn login(login: &String, password: &String) -> Result<(LoginData, Client), String> {
    ...
    let response = client
    .post(LOGIN_LINK)
    .form(&[("login", &login_data.login), ("password", &login_data.password)])
    .send()
    .await;
    ...
    let has_user_name_cookie = response.cookies().any(|c| {
        c.name() == "user_name"
    });
...
fn get_task_list(lv_mods_store: Arc<Mutex<gtk::ListStore>>, config: &mut ConfigObject, mod_list: &Vec<JsonModTemplate>) -> Vec<JsonModTemplate> {
    let mut task_list = vec![];
    let store = lv_mods_store.lock().unwrap();
    for (index, entry) in mod_list.iter().enumerate() {        
        let iter = store.get_iter_from_string(index.to_string().as_str()).unwrap();
...
*/
async fn install_mods(client: Client, lv_mods_store: Arc<Mutex<gtk::ListStore>>, config: &mut ConfigObject, mod_list: &Vec<JsonModTemplate>) {
    ...
    let install_thread = Arc::new(Mutex::new(install_thread::InstallThread::new(client, task_list)));
    ...
    if error_list.len() == 0 {
        display_summary(&format!("{} mods installed successfully.", successfully_installed_mods.len()));
    }
    else {
...
#[tokio::main]
async fn main() -> /*Result<(), String>*/ reqwest::Result<()> {
    if gtk::init().is_err() {
    ...
    while let Err(error) = &assetto_path {
        let dialog = MessageDialog::new(None::<&Window>, DialogFlags::MODAL, MessageType::Error, ButtonsType::YesNo, format!("{}.\nDo you want to try again?\n\"No\" will close the program.", error).as_str());
    ...
    let lv_mods_store: Arc<Mutex<gtk::ListStore>> = Arc::new(Mutex::new(builder.get_object("lv_mods_store").unwrap()));
    let lv_mods_toggle_box: gtk::CellRendererToggle = builder.get_object("lv_mods_toggle_box").unwrap();
    ...
    let window: Arc<Mutex<gtk::Window>> = Arc::new(Mutex::new(builder.get_object("window1").unwrap()));

    let window_event_clone = window.clone();
    window_event_clone.lock().unwrap().connect_delete_event(|wind, _| {
        gtk::main_quit();
        Inhibit(false)
    });
